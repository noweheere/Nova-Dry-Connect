    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button>
          ...
          <span>{t('logbook.addManualEntry')}</span>
        </Button>
      </div>
      ...
      {batches.length === 0 ? (
        <Card className="text-center text-gray-400 py-12">
            ...
            <h3 className="mt-2 text-lg font-medium text-white">{t('logbook.noBatchesTitle')}</h3>
            <p className="mt-1 text-sm">{t('logbook.noBatchesBody')}</p>
        </Card>
      ) : (
        <Card>
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div className="flex-1 mb-4 md:mb-0">
                        ...
                        <p className="text-sm text-gray-400">
                            {t('logbook.ranOn')} {new Date(batch.startTime).toLocaleDateString()} {t('logbook.with')} "{batch.recipe.name}"
                        </p>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm w-full md:w-auto md:flex-shrink-0 md:max-w-2xl">
                        <div className="bg-gray-700/50 p-3 rounded-lg">
                            <p className="text-gray-300 font-medium">{t('logbook.quantity')}</p>
                            <p className="text-white">{batch.quantity} g</p>
                        </div>
                        <div className="bg-gray-700/50 p-3 rounded-lg">
                            <p className="text-gray-300 font-medium">{t('logbook.sieves')}</p>
                            <p className="text-white truncate">{batch.washInfo.sievesUsed.join(', ')}</p>
                        </div>
                         <div className="bg-gray-700/50 p-3 rounded-lg">
                            <p className="text-gray-300 font-medium">{t('logbook.washCycles')}</p>
                            <p className="text-white">{batch.washInfo.washCycles}</p>
                        </div>
                        <div className="bg-gray-700/50 p-3 rounded-lg">
                            <p className="text-gray-300 font-medium">{t('logbook.trayType')}</p>
                            <p className="text-white">{batch.trayType}</p>
                        </div>
                    </div>
                </div>
                 <div className="mt-4">
                     <p className="text-sm text-gray-300 font-medium">{t('logbook.notes')}:</p>
                     <p className="text-sm text-gray-400 italic bg-gray-900/50 p-2 rounded-md">{batch.notes || t('logbook.noNotes')}</p>
                 </div>
              </li>
            ))}
          </ul>
