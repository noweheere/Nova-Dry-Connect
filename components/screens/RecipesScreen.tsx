  ...
  return (
    <div className="space-y-6">
       <div className="flex justify-between items-center">
         <h1 className="text-3xl font-bold text-white">{t('recipes.title')}</h1>
         <Button>
            <Icon path="M12 4.5v15m7.5-7.5h-15" className="h-5 w-5" />
            <span>{t('recipes.newRecipe')}</span>
         </Button>
       </div>
       ...
       {!dryerStatus.isConnected && (
            <div className="bg-yellow-900/50 border border-yellow-700 text-yellow-300 px-4 py-3 rounded-lg" role="alert">
                <p className="font-bold">{t('recipes.disconnectedWarningTitle')}</p>
                <p className="text-sm">{t('recipes.disconnectedWarningBody')}</p>
            </div>
        )}
          ...
          <Card key={recipe.id} className="flex flex-col justify-between">
                ...
                {recipe.steps.map((step, index) => (
                  <div key={step.id} className="flex justify-between items-center text-sm">
                    <span className="text-gray-300">
                      {t('common.step')} {index + 1}: {step.name}
                    </span>
            ...
            <div className="mt-6 flex items-center justify-between">
              ...
              <Button
                ...
                title={!canStartProcess ? t('recipes.startProcessTooltip') : ''}
              >
